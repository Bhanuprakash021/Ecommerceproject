...
from ecommerceapp.forms import RegistrationForm, LoginForm, ProductForm, CommentForm, LikeOrDisLikeForm
from ecommerceapp.models import RegistrationModel, ProductModel, CommentModel, LikeOrDisLikeModel, SearchHistoryModel, \
    TransactionModel
from ecommerceapp.service import getAllProducts
from ecommerceapp.spamservice import isspamuserbasedonproduct
...
def spam(request):
    print("in spam")
    users=isspamuserbasedonproduct(request.GET["pid"])
    print("after spam")
...
def deleteuser(request):
    ...
    for comment in CommentModel.objects.filter(user=userid):
        CommentModel.objects.filter(id=comment.id).delete()

    for likedislike in LikeOrDisLikeModel.objects.filter(user=userid):
        LikeOrDisLikeModel.objects.filter(id=likedislike.id).delete()

    return render(request, 'products.html',
                  {'message': "we will deliver you product soon", "products": getAllProducts()})
